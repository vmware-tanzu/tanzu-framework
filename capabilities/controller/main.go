...
import (
	...
	"github.com/vmware-tanzu/tanzu-framework/capabilities/controller/pkg/capabilities/discovery"
...
func main() {
	...
	if err = (&discovery.CapabilityReconciler{
		Client: mgr.GetClient(),
		Log:    ctrl.Log.WithName("controllers").WithName("Capability"),
		Scheme: mgr.GetScheme(),
		Host:   mgr.GetConfig().Host,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "Capability", "apigroup", "discovery")
		os.Exit(1)
	}
