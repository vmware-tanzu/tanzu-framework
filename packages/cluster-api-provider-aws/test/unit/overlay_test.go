...
var _ = Describe("Cluster-API Provider AWS Ytt Templating", func() {
	var paths []string
	var values string
  ...
  httpProxy: http://127.0.0.1:3124`
		...
		It("adds HTTP_PROXY environment variable to the deployment container", func() {
			...
			Expect(docs[0]).To(HaveYAMLPathWithValue(
				"$.spec.template.spec.containers[0].env[?(@.name=='AWS_SHARED_CREDENTIALS_FILE')].value",
