...
build-plugin-admin-%:
	...
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile --version $(BUILD_VERSION) --ldflags "$(LD_FLAGS) -X 'github.com/vmware-tanzu/tanzu-framework/cli/core/pkg/config.DefaultStandaloneDiscoveryType=${DISCOVERY_TYPE}'" --tags "${BUILD_TAGS}" --path .. --artifacts $(ARTIFACTS_ADMIN_DIR)/${OS}/${ARCH}/cli --target ${OS}_${ARCH})
...
build-cli-%: prep-build-cli
	...
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile --version $(BUILD_VERSION) --ldflags "$(LD_FLAGS) -X 'github.com/vmware-tanzu/tanzu-framework/cli/core/pkg/config.DefaultStandaloneDiscoveryType=${DISCOVERY_TYPE}'" --tags "${BUILD_TAGS}" --path "../../plugin" --artifacts $(ARTIFACTS_DIR)/${OS}/${ARCH}/cli --target  ${OS}_${ARCH})
...
build-cli-local: configure-buildtags-embedproviders build-cli-local-${GOHOSTOS}-${GOHOSTARCH} publish-plugins-local ## Build Tanzu CLI with local standalone discovery. cluster and management-cluster plugins are built with embedded providers.
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile --version $(BUILD_VERSION) --ldflags "$(LD_FLAGS) -X 'github.com/vmware-tanzu/tanzu-framework/cli/core/pkg/config.DefaultStandaloneDiscoveryType=local'" --tags "${BUILD_TAGS}" --path .. --artifacts $(ARTIFACTS_ADMIN_DIR)/${GOHOSTOS}/${GOHOSTARCH}/cli --target local)
...
publish-plugins-all-local: ## Publish CLI plugins locally for all supported os-arch
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . publish --type local --plugins "$(PLUGINS)" --version $(BUILD_VERSION) --os-arch "${ENVS}" --local-output-discovery-dir "$(TANZU_PLUGIN_PUBLISH_PATH)/discovery/standalone" --local-output-distribution-dir "$(TANZU_PLUGIN_PUBLISH_PATH)/distribution" --input-artifact-dir $(ARTIFACTS_DIR))
...
publish-admin-plugins-all-local: ## Publish CLI admin plugins locally for all supported os-arch
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . publish --type local --plugins "$(ADMIN_PLUGINS)" --version $(BUILD_VERSION) --os-arch "${ENVS}" --local-output-discovery-dir "$(TANZU_PLUGIN_PUBLISH_PATH)/discovery/admin" --local-output-distribution-dir "$(TANZU_PLUGIN_PUBLISH_PATH)/distribution" --input-artifact-dir $(ARTIFACTS_ADMIN_DIR))

.PHONY: publish-plugins-local
publish-plugins-local: ## Publish CLI plugins locally for current host os-arch only
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . publish --type local --plugins "$(PLUGINS)" --version $(BUILD_VERSION) --os-arch "${GOHOSTOS}-${GOHOSTARCH}" --local-output-discovery-dir "$(TANZU_PLUGIN_PUBLISH_PATH)/discovery/standalone" --local-output-distribution-dir "$(TANZU_PLUGIN_PUBLISH_PATH)/distribution" --input-artifact-dir $(ARTIFACTS_DIR))
...
publish-plugins-local-%: ## Publish CLI plugins to local directory that can be shared. Configure TANZU_PLUGIN_PUBLISH_PATH, PLUGINS, ARTIFACTS_DIR, DISCOVERY_NAME variables
	...
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . publish --type local --plugins "$(PLUGINS)" --version $(BUILD_VERSION) --os-arch "${OS}-${ARCH}" --local-output-discovery-dir "$(TANZU_PLUGIN_PUBLISH_PATH)/${OS}-${ARCH}-$(DISCOVERY_NAME)/discovery/$(DISCOVERY_NAME)" --local-output-distribution-dir "$(TANZU_PLUGIN_PUBLISH_PATH)/${OS}-${ARCH}-$(DISCOVERY_NAME)/distribution" --input-artifact-dir $(ARTIFACTS_DIR))
...
publish-admin-plugins-local: ## Publish CLI admin plugins locally for current host os-arch only
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . publish --type local --plugins "$(ADMIN_PLUGINS)" --version $(BUILD_VERSION) --os-arch "${GOHOSTOS}-${GOHOSTARCH}" --local-output-discovery-dir "$(TANZU_PLUGIN_PUBLISH_PATH)/discovery/admin" --local-output-distribution-dir "$(TANZU_PLUGIN_PUBLISH_PATH)/distribution" --input-artifact-dir $(ARTIFACTS_ADMIN_DIR))


.PHONY: publish-plugins-all-oci
publish-plugins-all-oci: ## Publish CLI plugins as OCI image for all supported os-arch
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . publish --type oci --plugins "$(STANDALONE_PLUGINS)" --version $(BUILD_VERSION) --os-arch "${ENVS}" --oci-discovery-image ${OCI_REGISTRY}/tanzu-plugins/discovery/standalone:${BUILD_VERSION} --oci-distribution-image-repository ${OCI_REGISTRY}/tanzu-plugins/distribution/ --input-artifact-dir $(ARTIFACTS_DIR))
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . publish --type oci --plugins "$(CONTEXTAWARE_PLUGINS)" --version $(BUILD_VERSION) --os-arch "${ENVS}" --oci-discovery-image ${OCI_REGISTRY}/tanzu-plugins/discovery/context:${BUILD_VERSION} --oci-distribution-image-repository ${OCI_REGISTRY}/tanzu-plugins/distribution/ --input-artifact-dir $(ARTIFACTS_DIR))
...
publish-admin-plugins-all-oci: ## Publish CLI admin plugins as OCI image for all supported os-arch
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . publish --type oci --plugins "$(ADMIN_PLUGINS)" --version $(BUILD_VERSION) --os-arch "${ENVS}" --oci-discovery-image ${OCI_REGISTRY}/tanzu-plugins/discovery/admin:${BUILD_VERSION} --oci-distribution-image-repository ${OCI_REGISTRY}/tanzu-plugins/distribution/ --input-artifact-dir $(ARTIFACTS_ADMIN_DIR))

.PHONY: publish-plugins-oci
publish-plugins-oci: ## Publish CLI plugins as OCI image for current host os-arch only
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . publish --type oci --plugins "$(STANDALONE_PLUGINS)" --version $(BUILD_VERSION) --os-arch "${GOHOSTOS}-${GOHOSTARCH}" --oci-discovery-image ${OCI_REGISTRY}/tanzu-plugins/discovery/standalone:${BUILD_VERSION} --oci-distribution-image-repository ${OCI_REGISTRY}/tanzu-plugins/distribution/ --input-artifact-dir $(ARTIFACTS_DIR))
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . publish --type oci --plugins "$(CONTEXTAWARE_PLUGINS)" --version $(BUILD_VERSION) --os-arch "${GOHOSTOS}-${GOHOSTARCH}" --oci-discovery-image ${OCI_REGISTRY}/tanzu-plugins/discovery/context:${BUILD_VERSION} --oci-distribution-image-repository ${OCI_REGISTRY}/tanzu-plugins/distribution/ --input-artifact-dir $(ARTIFACTS_DIR))
...
publish-admin-plugins-oci: ## Publish CLI admin plugins as OCI image for current host os-arch only
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . publish --type oci --plugins "$(ADMIN_PLUGINS)" --version $(BUILD_VERSION) --os-arch "${GOHOSTOS}-${GOHOSTARCH}" --oci-discovery-image ${OCI_REGISTRY}/tanzu-plugins/discovery/admin:${BUILD_VERSION} --oci-distribution-image-repository ${OCI_REGISTRY}/tanzu-plugins/distribution/ --input-artifact-dir $(ARTIFACTS_ADMIN_DIR))
...
build-cli-mocks: ## Build Tanzu CLI mocks
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile $(addprefix --target ,$(subst -,_,${ENVS})) --version 0.0.1 --ldflags "$(LD_FLAGS)" --tags "${BUILD_TAGS}" --path ../../../../test/cli/mock/plugin-old --artifacts $(ROOT_DIR)/test/cli/mock/artifacts-old)
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile $(addprefix --target ,$(subst -,_,${ENVS})) --version 0.0.2 --ldflags "$(LD_FLAGS)" --tags "${BUILD_TAGS}" --path ../../../../test/cli/mock/plugin-new --artifacts $(ROOT_DIR)/test/cli/mock/artifacts-new)
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile $(addprefix --target ,$(subst -,_,${ENVS})) --version 0.0.3 --ldflags "$(LD_FLAGS)" --tags "${BUILD_TAGS}" --path ../../../../test/cli/mock/plugin-alt --artifacts $(ROOT_DIR)/test/cli/mock/artifacts-alt)
...
install-cli-plugins-without-discovery: set-unstable-versions set-context-aware-cli-for-plugins ## Install Tanzu CLI plugins when context-aware discovery is disabled
	...
	cd ./cli/core && TANZU_CLI_NO_INIT=true $(GO) run -ldflags "$(LD_FLAGS)" ./cmd/tanzu/main.go \
		plugin install all --local $(ARTIFACTS_ADMIN_DIR)/$(GOHOSTOS)/$(GOHOSTARCH)/cli
	cd ./cli/core && TANZU_CLI_NO_INIT=true $(GO) run -ldflags "$(LD_FLAGS)" ./cmd/tanzu/main.go \
		test fetch --local $(ARTIFACTS_DIR)/$(GOHOSTOS)/$(GOHOSTARCH)/cli --local $(ARTIFACTS_ADMIN_DIR)/$(GOHOSTOS)/$(GOHOSTARCH)/cli
...
install-cli-plugins-from-local-discovery: clean-catalog-cache clean-cli-plugins set-context-aware-cli-for-plugins configure-admin-plugins-discovery-source-local ## Install Tanzu CLI plugins from local discovery
	cd ./cli/core && TANZU_CLI_NO_INIT=true $(GO) run -ldflags "$(LD_FLAGS) -X 'github.com/vmware-tanzu/tanzu-framework/pkg/v1/config.DefaultStandaloneDiscoveryType=local'" ./cmd/tanzu/main.go plugin sync
...
install-cli-plugins-from-oci-discovery: clean-catalog-cache clean-cli-plugins set-context-aware-cli-for-plugins ## Install Tanzu CLI plugins from OCI discovery
	cd ./cli/core && TANZU_CLI_NO_INIT=true $(GO) run -ldflags "$(LD_FLAGS) -X 'github.com/vmware-tanzu/tanzu-framework/pkg/v1/config.DefaultStandaloneDiscoveryType=oci'" ./cmd/tanzu/main.go plugin sync
...
release-%: ## Create release for a platform
	...
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile --version $(BUILD_VERSION) --ldflags "$(LD_FLAGS) -X 'github.com/vmware-tanzu/tanzu-framework/cli/core/pkg/config.DefaultStandaloneDiscoveryType=oci'" --tags "${BUILD_TAGS}" --path .. --artifacts $(ARTIFACTS_ADMIN_DIR)/${OS}/${ARCH}/cli --target ${OS}_${ARCH})
	$(ROOT_DIR)/hack/embed-pinniped-binary.sh go ${OS} ${ARCH} ${PINNIPED_VERSIONS}
	(cd ./cmd/cli/plugin-admin/builder && $(GO) run . cli compile --version $(BUILD_VERSION) --ldflags "$(LD_FLAGS) -X 'github.com/vmware-tanzu/tanzu-framework/cli/core/pkg/config.DefaultStandaloneDiscoveryType=oci'" --tags "${BUILD_TAGS}" --path "../../plugin" --artifacts $(ARTIFACTS_DIR)/${OS}/${ARCH}/cli --target  ${OS}_${ARCH})
