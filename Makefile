...
.PHONY: generate-package-config ## Generate the default package config CR e.g. make generate-package-config apiGroup=cni.tanzu.vmware.com kind=AntreaConfig version=v1alpha1 tkr=v1.23.3---vmware.1-tkg.1 namespace=tkg-system iaas=vsphere
generate-package-config:
	@cd addons/config && \
        ./hack/test.sh verifyAddonConfigTemplateForGVR ${apiGroup} ${version} $(shell echo $(kind) | tr A-Z a-z) && \
		$(YTT) --ignore-unknown-comments -f templates/${apiGroup}/${version}/$(shell echo $(kind) | tr A-Z a-z).yaml -v TKR_VERSION=${tkr} -v GLOBAL_NAMESPACE=$(or $(namespace),"tkg-system") -v IAAS=${iaas} ;\
