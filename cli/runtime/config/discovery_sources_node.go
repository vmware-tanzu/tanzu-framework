...
func setDiscoverySource(discoverySourcesNode *yaml.Node, discoverySource configapi.PluginDiscovery, patchStrategyOpts ...nodeutils.PatchStrategyOpts) (persist bool, err error) {
	...
	// findDiscoverySourceIndexOfAnyType
	...
	for _, discoverySourceNode := range discoverySourcesNode.Content {
		...
		if discoverySourceIndexOfAnyType != -1 && discoverySourceIndexOfExactType != -1 {
			...
			if nameIdx != -1 {
				...
				isNameDefaultOrDefaultLocal := checkDefaultAndDefaultLocal(discoverySourceNode.Content[discoverySourceIndexOfAnyType].Content[nameIdx].Value, newOrUpdatedDiscoverySourceName)
		...
		// If discoverySourceIndex == -1 i.e. not found  then append the discoverySourceNode
...
func checkDefaultAndDefaultLocal(existingDiscoverySource, newOrUpdatedDiscoverySource string) bool {
