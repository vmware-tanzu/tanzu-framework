...
func processYamlObjectAndAddToMap(value interface{}, clusterAttributePath string, variablesMap map[string]interface{}) error {
	var err error
	switch value := value.(type) {
	case []interface{}:
		err = processYamlObjectArrayInterfaceType(value, clusterAttributePath, variablesMap)
	case []map[string]interface{}:
		for index := range value {
			err = processYamlObjectAndAddToMap(value[index], clusterAttributePath, variablesMap)
			if err != nil {
				return err
			}
		}
	case map[string]interface{}:
		for key := range value {
			...
			if clusterAttributePath+"."+nextLevelName == "spec.topology.variables.proxy.noProxy" {
				variablesMap[clusterAttributePath+"."+nextLevelName] = nextLevelVal
			} else {
				err = processYamlObjectAndAddToMap(nextLevelVal, clusterAttributePath+"."+nextLevelName, variablesMap)
				if err != nil {
					return err
				}
			}
	...
	case interface{}:
		if _, ok := variablesMap[clusterAttributePath]; ok {
			...
			variablesMap[clusterAttributePath] = value
		}
	default:
		if value != nil {
...
func processYamlObjectArrayInterfaceType(value []interface{}, clusterAttributePath string, variablesMap map[string]interface{}) error {
	var err error
	for index := range value {
		if clusterAttributePath == constants.TopologyVariablesSubnets || clusterAttributePath == constants.TopologyVariablesNodes || clusterAttributePath == constants.TopologyWorkersMachineDeployments {
			err = processYamlObjectAndAddToMap(value[index], clusterAttributePath+"."+strconv.Itoa(index), variablesMap)
