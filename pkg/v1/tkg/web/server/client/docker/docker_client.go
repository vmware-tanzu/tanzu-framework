...
func (a *Client) ImportTKGConfigForDocker(params *ImportTKGConfigForDockerParams) (*ImportTKGConfigForDockerOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewImportTKGConfigForDockerParams()
	}

	result, err := a.transport.Submit(&runtime.ClientOperation{
		ID:                 "importTKGConfigForDocker",
		Method:             "POST",
		PathPattern:        "/api/providers/docker/config/import",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http", "https"},
		Params:             params,
		Reader:             &ImportTKGConfigForDockerReader{formats: a.formats},
		Context:            params.Context,
		Client:             params.HTTPClient,
	})
	if err != nil {
	...
	success, ok := result.(*ImportTKGConfigForDockerOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for importTKGConfigForDocker: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
