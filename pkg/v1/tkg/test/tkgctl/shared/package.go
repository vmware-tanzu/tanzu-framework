...
func getPackagesFromCB(ctx context.Context, clusterBootstrap *runtanzuv1alpha3.ClusterBootstrap, mccl, wccl client.Client, mcClusterName, mcClusterNamespace, wcClusterName, wcClusterNamespace, infrastructureName string, isManagementCluster bool) ([]kapppkgv1alpha1.Package, error) {
	var (
		packages        []kapppkgv1alpha1.Package
...
func CheckClusterCB(ctx context.Context, mccl, wccl client.Client, mcClusterName, mcClusterNamespace, wcClusterName, wcClusterNamespace, infrastructureName string, isManagementCluster bool) error {
	...
	// in tkgs infrastructure, CB only exists in the supervisor cluster
	if infrastructureName == "tkgs" || isManagementCluster {
		clusterBootstrap = getClusterBootstrap(ctx, mccl, mcClusterNamespace, mcClusterName)
	} else {
		clusterBootstrap = getClusterBootstrap(ctx, wccl, wcClusterNamespace, wcClusterName)
	}

	By(fmt.Sprintf("Verify clusterbootstrap matches clusterbootstraptemplate"))
	verifyClusterBootstrap(ctx, mccl, clusterBootstrap, clusterBootstrap.Status.ResolvedTKR, infrastructureName)
	...
	for _, pkg := range packages {
		if strings.Contains(pkg.Name, "kapp-controller") {
			...
			// we skip verifying package installs in tkgs supervisor cluster
			// in tkgs guest cluster, we skip verifying package install for capabilities package as it doesn't reconcile
			// TODO: Verify the capabilities package install when it is fixed to reconcile
			if infrastructureName != "tkgs" || (!isManagementCluster && pkg.Name != "capabilities") {
	...
	// Skip verifying package install status in CB for tkgs guest cluster as there is no CB
	if infrastructureName != "tkgs" || isManagementCluster {
...
func verifyClusterBootstrap(ctx context.Context, c client.Client, clusterBootstrap *runtanzuv1alpha3.ClusterBootstrap, tkrName string, infrastructureName string) {
	resolvedTKr := clusterBootstrap.Status.ResolvedTKR
	Expect(resolvedTKr).NotTo(BeEmpty())

	clusterBootstrapTemplate := getClusterBootstrapTemplate(ctx, c, resolvedTKr, infrastructureName)
...
func verifyPackageInstallStatusinClusterBootstrapStatus(ctx context.Context, mccl client.Client, mcClusterName, mcClusterNamespace, wcClusterName, wcClusterNamespace string, infrastructureName string, isManagementCluster bool, packages []kapppkgv1alpha1.Package) {
	Eventually(func() bool {
		...
		for _, pkg := range packages {
			...
			// TODO: Temporarily skip verifying capabilities package install for tkgs supervisor cluster
...
func getClusterBootstrapTemplate(ctx context.Context, k8sClient client.Client, tkrName string, infrastructureName string) *runtanzuv1alpha3.ClusterBootstrapTemplate {
	var objKey client.ObjectKey

	clusterBootstrapTemplate := &runtanzuv1alpha3.ClusterBootstrapTemplate{}
	if infrastructureName == "tkgs" {
		objKey = client.ObjectKey{Name: tkrName, Namespace: constants.TKGSClusterClassNamespace}
	} else {
