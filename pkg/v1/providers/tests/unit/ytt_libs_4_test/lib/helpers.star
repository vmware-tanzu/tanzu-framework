...
def get_bom_data_for_tkr_name():
    ...
    for bom_entry in data.values.boms:
      ...
      if value == None or value == "":
        assert.fail("missing the release field from " + bom_entry.bom_name)
      end
      value = getattr(value, "version", None)
      if value == None or value == "":
        assert.fail("missing the release.version field from " + bom_entry.bom_name)
      end
    end
    ...
    assert.fail("unable to get BoM file for the TanzuKubernetesRelease version: " + data.values.KUBERNETES_RELEASE + " (looking for release.version=" + tkr_version +")")
end
