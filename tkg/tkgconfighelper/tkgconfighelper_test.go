...
const (
	...
	k8sVersion1dot26dot2vmware1  = "v1.26.2+vmware.1"
...
var _ = Describe("ValidateK8sVersionSupport", func() {
	...
	Context("when k8s version is not supported by management cluster", func() {
		...
		Context("mgmtClusterVersion= v3.6.0, kubernetesVersion=v1.21.1+vmware.1", func() {
			...
			It("should return error", func() {
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("only [v1.0 v1.1 v1.2 v1.3 v1.4 v1.5 v1.6 v1.7 v2.1 v2.2 v2.3] management cluster versions are supported with current version of TKG CLI. Please upgrade TKG CLI to latest version if you are using it on latest version of management cluster."))
			})
		})
	})
	...
	Context("when k8s version is supported by management cluster", func() {
		...
		Context("mgmtClusterVersion= v2.3.0, kubernetesVersion=v1.26.2+vmware.1", func() {
			BeforeEach(func() {
				mgmtClusterVersion = "v2.3.0"
				kubernetesVersion = k8sVersion1dot26dot2vmware1
			})
			It("should not return error", func() {
				Expect(err).NotTo(HaveOccurred())
			})
