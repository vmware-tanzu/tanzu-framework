...
func WaitForManagementPackages(clusterClient clusterclient.Client, packageInstallTimeout time.Duration) error {
	...
	for _, packageName := range packageInstallNames {
		pn := packageName
		// skip waiting for ako packageinstall because ako add-on secret won't be
		// generated by ako operator until cluster object created.
		if pn == akoPackageInstallName {
			continue
		}
