...
func init() {
	...
	command.DeprecateFlagWithAlternative(describeCmd, "disable-no-echo", "1.6.0", "--show-details")
	...
	command.DeprecateFlagWithAlternative(describeCmd, "disable-grouping", "1.6.0", "--show-group-members")
...
func treeViewInner(prefix string, tbl *uitable.Table, objs *clusterctltree.ObjectTree, obj crtclient.Object) {
	...
	if clusterctltree.IsGroupObject(obj) {
		...
		if len(items) <= GroupItemLimit {
			...
			v.message = gray.Sprintf("See %s, ...", strings.Join(items[:GroupItemLimit], clusterctltree.GroupItemsSeparator))
		}
	}
