...
func (webhook *ClusterBootstrap) ValidateClusterBootstrapPackage(ctx context.Context, pkg *runv1alpha3.ClusterBootstrapPackage, namespace string, fldPath *field.Path) *field.Error {
	if pkg == nil {
		return field.Invalid(fldPath, pkg, "package can't be nil")
	}

	// The package URL in refName must be valid
	...
	if pkg.ValuesFrom == nil {
		...
		// TODO: Question: Do we allow more than one field from valuesFrom to be present?
		// Currently, we don't allow one field from valuesFrom to be present
...
func (webhook *ClusterBootstrap) ValidateUpdate(ctx context.Context, oldObj, newObj runtime.Object) error {
	...
	validatePackageSpecAndUpgrade := func(ctx context.Context, old, new *runv1alpha3.ClusterBootstrapPackage, namespace string, fldPath *field.Path) *field.Error {
	...
	if len(oldClusterBootstrap.Spec.CNIs) == 1 && len(newClusterBootstrap.Spec.CNIs) == 1 {
		if err := validatePackageSpecAndUpgrade(ctx, oldClusterBootstrap.Spec.CNIs[0], newClusterBootstrap.Spec.CNIs[0], namespace, getFieldPath("cnis")); err != nil {
			allErrs = append(allErrs, err)
		}
	} else {
	...
	if err := validatePackageSpecAndUpgrade(ctx, oldClusterBootstrap.Spec.CSI, newClusterBootstrap.Spec.CSI, namespace, getFieldPath("csi")); err != nil {
		allErrs = append(allErrs, err)
	}

	if err := validatePackageSpecAndUpgrade(ctx, oldClusterBootstrap.Spec.CPI, newClusterBootstrap.Spec.CPI, namespace, getFieldPath("cpi")); err != nil {
		allErrs = append(allErrs, err)
	}

	if err := validatePackageSpecAndUpgrade(ctx, oldClusterBootstrap.Spec.Kapp, newClusterBootstrap.Spec.Kapp, namespace, getFieldPath("kapp")); err != nil {
		allErrs = append(allErrs, err)
	}
	...
	if newClusterBootstrap.Spec.AdditionalPackages != nil {
		for _, pkg := range newClusterBootstrap.Spec.AdditionalPackages {
			...
			newCarvelPkg, err := webhook.GetCarvelPackageCRFromRefName(ctx, pkg.RefName)
			if err != nil {
			...
			if newAdditionalPkgMap[newCarvelPkg.Spec.RefName] != nil {
				allErrs = append(allErrs, field.Invalid(addtionalPkgFldPath.Child("refName"), newCarvelPkg.Spec.RefName, "more than one additional packages share the same refName"))
				continue
			}
			newAdditionalPkgMap[newCarvelPkg.Spec.RefName] = pkg
		}
	}
	...
	if oldClusterBootstrap.Spec.AdditionalPackages != nil {
		for _, pkg := range oldClusterBootstrap.Spec.AdditionalPackages {
			oldCarvelPkg, err := webhook.GetCarvelPackageCRFromRefName(ctx, pkg.RefName)
			if err != nil {
				allErrs = append(allErrs, field.Invalid(addtionalPkgFldPath.Child("refName"), pkg.RefName, err.Error()))
				continue
			}
			if newAdditionalPkgMap[oldCarvelPkg.Spec.RefName] == nil {
			...
			if err := webhook.ValidateClusterBootstrapPackageUpdate(ctx, pkg, newAdditionalPkgMap[oldCarvelPkg.Spec.RefName], addtionalPkgFldPath); err != nil {
				allErrs = append(allErrs, err)
			}
		}
	}
...
func (webhook *ClusterBootstrap) ValidateClusterBootstrapPackageUpdate(ctx context.Context, old, new *runv1alpha3.ClusterBootstrapPackage, fldPath *field.Path) *field.Error {
	...
	newCarvelPkg, err := webhook.GetCarvelPackageCRFromRefName(ctx, new.RefName)
	if err != nil {
		return field.Invalid(fldPath.Child("refName"), new.RefName, err.Error())
	}

	oldCarvelPkg, err := webhook.GetCarvelPackageCRFromRefName(ctx, old.RefName)
	if err != nil {
	...
	if newCarvelPkg.Spec.RefName != oldCarvelPkg.Spec.RefName {
	...
	newPkgSemver, err := versions.NewRelaxedSemver(newCarvelPkg.Spec.Version)
	if err != nil {
		retErr := errors.Wrap(err, "new package version is invalid")
		return field.Invalid(fldPath.Child("refName"), new.RefName, retErr.Error())
	}
	oldPkgSemver, err := versions.NewRelaxedSemver(oldCarvelPkg.Spec.Version)
	if err != nil {
	...
	if old.ValuesFrom != nil && new.ValuesFrom != nil {
		...
		if old.ValuesFrom.ProviderRef != nil && new.ValuesFrom.ProviderRef != nil {
			if old.ValuesFrom.ProviderRef.APIGroup != new.ValuesFrom.ProviderRef.APIGroup ||
				...
				return field.Invalid(fldPath.Child("valuesFrom"), new.ValuesFrom.ProviderRef, "Group and Kind change in ProviderRef is not allowed")
			}
		}
...
func (webhook *ClusterBootstrap) GetCarvelPackageCRFromRefName(ctx context.Context, refName string) (*kapppkgv1alpha1.Package, error) {
	pkg := &kapppkgv1alpha1.Package{}
	// TODO: use configurable BootstrapSystemNamespace when https://github.com/vmware-tanzu/tanzu-framework/pull/1679 is merged
	newPkgKey := client.ObjectKey{Namespace: constants.TKGSystemNS, Name: refName}
	err := webhook.Client.Get(ctx, newPkgKey, pkg)
	if err != nil {
		return nil, errors.Wrap(err, "package is not found")
	}
	return pkg, nil
}
