...
var _ = Describe("Addon util test cases", func() {
	Context("ParseStringForLabel", func() {
		...
		When("label be of long size and with non-alphanumeric trailing '-' characters just before index 62", func() {
			BeforeEach(func() {
				label = "foobar1.very.long.name.for.a.label.exceeding.63.characters.e---ple.com.1.17.2"
				result = ParseStringForLabel(label)
			})
			It("should parse label", func() {
				Expect(result).To(Equal("foobar1.very.long.name.for.a.label.exceeding.63.characters.e"))
			})
		})

		When("label be of long size and with non-alphanumeric trailing '.' character just before index 62", func() {
			BeforeEach(func() {
				label = "foobar1.very.long.name.for.a.label.exceeding.63.characters.exa.ple.com.1.17.2"
				result = ParseStringForLabel(label)
			})
			It("should parse label", func() {
				Expect(result).To(Equal("foobar1.very.long.name.for.a.label.exceeding.63.characters.exa"))
			})
		})
		When("label be of long size and with non-alphanumeric trailing '_' character just before index 62", func() {
			BeforeEach(func() {
				label = "foobar1.very.long.name.for.a.label.exceeding.63.characters.e___ple.com.1.17.2--"
				result = ParseStringForLabel(label)
			})
			It("should parse label", func() {
				Expect(result).To(Equal("foobar1.very.long.name.for.a.label.exceeding.63.characters.e"))
