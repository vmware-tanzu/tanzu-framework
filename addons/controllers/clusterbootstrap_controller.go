...
// createOrPatchPackageMetadataOnRemote creates the PackageMetadata CR on remote cluster.
// createOrPatchPackageMetadataOnRemote returns a tuple: (<remote-package-metadata>, <error>)
func (r *ClusterBootstrapReconciler) createOrPatchPackageMetadataOnRemote(cluster *clusterapiv1beta1.Cluster,
	cbPkg *runtanzuv1alpha3.ClusterBootstrapPackage, clusterClient client.Client) (*kapppkgv1alpha1.PackageMetadata, error) {
	...
	if packageRefName == "" || err != nil {
		r.Log.Error(err, fmt.Sprintf("unable to fetch Package.Spec.RefName from Package %s/%s from management cluster", cluster.Namespace, cbPkg.RefName))
		return nil, err
	}

	localPackageMetadata := &kapppkgv1alpha1.PackageMetadata{}
	key := client.ObjectKey{Namespace: cluster.Namespace, Name: packageRefName}
	if err = r.aggregatedAPIResourcesClient.Get(r.context, key, localPackageMetadata); err != nil {
		// If there is an error to get the Carvel PackageMetadata CR from local cluster, nothing needs to be created/cloned on remote.
		// Let the reconciler try again.
		r.Log.Error(err, fmt.Sprintf("Error when getting PackageMetadata %s/%s from the management cluster",
			cluster.Namespace, packageRefName))
		return nil, err
	}

	remotePackageMetadata := &kapppkgv1alpha1.PackageMetadata{}
	remotePackageMetadata.SetName(localPackageMetadata.Name)
	// The PackageMetadata CR on remote cluster needs to be under configured system namespace
	remotePackageMetadata.SetNamespace(r.Config.SystemNamespace)
	_, err = controllerutil.CreateOrPatch(r.context, clusterClient, remotePackageMetadata, func() error {
		remotePackageMetadata.Spec = *localPackageMetadata.Spec.DeepCopy()
		return nil
	})
	if err != nil {
		r.Log.Error(err, fmt.Sprintf("unable to create or patch PackageMetadata resource %s/%s on cluster: %s/%s",
			remotePackageMetadata.Namespace, remotePackageMetadata.Name, cluster.Namespace, cluster.Name))
		return nil, err
	}
	return remotePackageMetadata, nil
}
...
func (r *ClusterBootstrapReconciler) createOrPatchAddonResourcesOnRemote(cluster *clusterapiv1beta1.Cluster,
	...
	if _, err = r.createOrPatchPackageMetadataOnRemote(cluster, cbPkg, clusterClient); err != nil {
		// PackageMetadata is not mandatory for the CRUD of package on cluster. It is only for informational purposes.
		// Hence just logging and not returning error if PackageMetadata is failed to be created
		r.Log.Info(fmt.Sprintf("Failed creating the PackageMetadata CR %s on cluster %s/%s due to %v", remotePackage.Spec.RefName, cluster.Namespace, cluster.Name, err))
	}
