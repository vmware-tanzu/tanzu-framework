...
func NewKappClientForContext(kubeCfgPath, kubeContext string) (Client, error) {
	...
	restConfig.QPS = packagedatamodel.DefaultQPS
	restConfig.Burst = packagedatamodel.DefaultBurst
...
func (c *client) addAnnotations(meta *v1.ObjectMeta, pkgPluginResourceCreationStatus *packagedatamodel.PkgPluginResourceCreationStatus) {
	if meta.Annotations == nil {
		meta.Annotations = make(map[string]string)
	}
	if pkgPluginResourceCreationStatus.IsServiceAccountCreated {
		meta.Annotations[packagedatamodel.TanzuPkgPluginAnnotation+"-"+packagedatamodel.KindClusterRole] = fmt.Sprintf(packagedatamodel.ClusterRoleName, meta.Name, meta.Namespace)
		meta.Annotations[packagedatamodel.TanzuPkgPluginAnnotation+"-"+packagedatamodel.KindClusterRoleBinding] = fmt.Sprintf(packagedatamodel.ClusterRoleBindingName, meta.Name, meta.Namespace)
		meta.Annotations[packagedatamodel.TanzuPkgPluginAnnotation+"-"+packagedatamodel.KindServiceAccount] = fmt.Sprintf(packagedatamodel.ServiceAccountName, meta.Name, meta.Namespace)
	}
	if pkgPluginResourceCreationStatus.IsSecretCreated {
		meta.Annotations[packagedatamodel.TanzuPkgPluginAnnotation+"-"+packagedatamodel.KindSecret] = fmt.Sprintf(packagedatamodel.SecretName, meta.Name, meta.Namespace)
	}
}
...
func (c *client) CreatePackageInstall(packageInstall *kappipkg.PackageInstall, pkgPluginResourceCreationStatus *packagedatamodel.PkgPluginResourceCreationStatus) error {
...
func (c *client) UpdatePackageInstall(packageInstall *kappipkg.PackageInstall, pkgPluginResourceCreationStatus *packagedatamodel.PkgPluginResourceCreationStatus) error {
...
func (c *client) GetSecretValue(secretName, namespace string) ([]byte, error) {
	...
	for _, value := range secret.Data {
		...
		if len(stringValue) < 3 {
			data = append(data, packagedatamodel.YamlSeparator...)
			data = append(data, "\n"...)
		}
