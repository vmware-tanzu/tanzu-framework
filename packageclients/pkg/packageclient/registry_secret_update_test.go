...
var _ = Describe("Update Secret", func() {
	...
	var (
		...
		opts             = packagedatamodel.RegistrySecretOptions{
			Export:     packagedatamodel.TypeBoolPtr{ExportToAllNamespaces: &t},
	...
	Context("failure in updating Secret due to Secret Get NotFound error", func() {
		BeforeEach(func() {
			...
			crtCtl.GetReturnsOnCall(0, apierrors.NewNotFound(schema.GroupResource{Resource: packagedatamodel.KindSecret}, testSecretName))
		})
	...
	Context("failure in updating the Secret due to SecretExport Create error", func() {
		BeforeEach(func() {
			...
			testSecret.Data[corev1.DockerConfigJsonKey] = dockerCfgContent
			crtCtl.UpdateReturnsOnCall(0, nil)
			crtCtl.GetReturnsOnCall(1, apierrors.NewNotFound(schema.GroupResource{Resource: packagedatamodel.KindSecretExport}, testSecretName))
	...
	Context("failure in updating the Secret with --export-to-all-namespaces=false due to SecretExport Delete error", func() {
		BeforeEach(func() {
			options.Export = packagedatamodel.TypeBoolPtr{ExportToAllNamespaces: &f}
			kappCtl = &fakes.KappClient{}
			crtCtl = &fakes.CrtClient{}
			kappCtl.GetClientReturns(crtCtl)
			Secret = testSecret
