...
func (p *pkgClient) UninstallPackage(o *packagedatamodel.PackageOptions, progress *packagedatamodel.PackageProgress) {
...
func (p *pkgClient) UninstallPackageSync(o *packagedatamodel.PackageOptions) error {
...
func (p *pkgClient) uninstallPackage(o *packagedatamodel.PackageOptions, progress *packagedatamodel.PackageProgress) {
	...
	if err = p.waitForResourceDeletion(o.PkgInstallName, o.Namespace, o.PollInterval, o.PollTimeout, progress.ProgressMsg, packagedatamodel.ResourceTypePackageInstall); err != nil {
		return
	}
...
func (p *pkgClient) deletePkgPluginCreatedResources(pkgInstall *kappipkg.PackageInstall, progress chan string) error { //nolint:gocyclo
	for k, v := range pkgInstall.GetAnnotations() {
		...
		resourceKind := strings.Split(split[1], packagedatamodel.TanzuPkgPluginPrefix+"-")
		...
		case packagedatamodel.KindSecret:
			if progress != nil {
				progress <- fmt.Sprintf("Deleting secret '%s'", objMeta.Name)
			}
			obj = &corev1.Secret{ObjectMeta: objMeta, TypeMeta: metav1.TypeMeta{Kind: packagedatamodel.KindSecret}}
		...
		case packagedatamodel.KindServiceAccount:
			if progress != nil {
				progress <- fmt.Sprintf("Deleting service account '%s'", objMeta.Name)
			}
			obj = &corev1.ServiceAccount{ObjectMeta: objMeta, TypeMeta: metav1.TypeMeta{Kind: packagedatamodel.KindServiceAccount}}
			if err := p.kappClient.GetClient().Delete(context.Background(), obj); err != nil {
				return errors.Wrap(err, "failed to delete ServiceAccount resource")
			}
		case packagedatamodel.KindClusterRole:
			if progress != nil {
				progress <- fmt.Sprintf("Deleting admin role '%s'", objMeta.Name)
			}
			obj = &rbacv1.ClusterRole{
				ObjectMeta: metav1.ObjectMeta{Name: v},
				TypeMeta:   metav1.TypeMeta{Kind: packagedatamodel.KindClusterRole},
			}
			if err := p.kappClient.GetClient().Delete(context.Background(), obj); err != nil {
				return errors.Wrap(err, "failed to delete ClusterRole resource")
			}
		case packagedatamodel.KindClusterRoleBinding:
			if progress != nil {
				progress <- fmt.Sprintf("Deleting role binding '%s'", objMeta.Name)
			}
			obj = &rbacv1.ClusterRoleBinding{
				ObjectMeta: metav1.ObjectMeta{Name: v},
				TypeMeta:   metav1.TypeMeta{Kind: packagedatamodel.KindClusterRoleBinding},
			}
			if err := p.kappClient.GetClient().Delete(context.Background(), obj); err != nil {
...
func (p *pkgClient) deletePackageInstall(o *packagedatamodel.PackageOptions) error {
...
func (p *pkgClient) deletePreviouslyInstalledResources(o *packagedatamodel.PackageOptions, progress chan string) error {
	var name string
	resourceAnnotation := fmt.Sprintf(packagedatamodel.TanzuPkgPluginResource, o.PkgInstallName, o.Namespace)

	name = fmt.Sprintf(packagedatamodel.ClusterRoleBindingName, o.PkgInstallName, o.Namespace)
	...
	name = fmt.Sprintf(packagedatamodel.ClusterRoleName, o.PkgInstallName, o.Namespace)
	...
	name = fmt.Sprintf(packagedatamodel.ServiceAccountName, o.PkgInstallName, o.Namespace)
...
func (p *pkgClient) deleteAnnotatedResource(resourceType string, obj crtclient.Object, objKey crtclient.ObjectKey, resourceAnnotation string, progress chan string) error {
	if err := p.kappClient.GetClient().Get(context.Background(), objKey, obj); err != nil {
		...
		for k, v := range o.GetAnnotations() {
			split := strings.Split(k, "/")
			if len(split) <= 1 || split[1] != packagedatamodel.TanzuPkgPluginPrefix {
				continue
			}
...
func progressCleanup(err error, progress *packagedatamodel.PackageProgress) {
	if err != nil {
		progress.Err <- err
	}
...
func (p *pkgClient) waitForResourceDeletion(name, namespace string, pollInterval, pollTimeout time.Duration, progress chan string, rscType packagedatamodel.ResourceType) error {
