...
var _ = Describe("Update Package", func() {
	...
	JustBeforeEach(func() {
		...
		go ctl.UpdatePackage(&options, progress, packagedatamodel.OperationTypeUpdate)
	...
	Context("failure in updating the installed package due to failure in updating the secret resource", func() {
		BeforeEach(func() {
			...
			testPkgInstall.Annotations[packagedatamodel.TanzuPkgPluginAnnotation+"-Secret"] = fmt.Sprintf(packagedatamodel.SecretName, options.PkgInstallName, options.Namespace)
			kappCtl = &fakes.KappClient{}
			crtCtl = &fakes.CrtClient{}
			kappCtl.GetClientReturns(crtCtl)
			kappCtl.GetPackageInstallReturnsOnCall(0, testPkgInstall, nil)
