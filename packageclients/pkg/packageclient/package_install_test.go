...
var _ = Describe("Install Package", func() {
	...
	JustBeforeEach(func() {
		...
		go ctl.InstallPackage(&options, progress, packagedatamodel.OperationTypeInstall)
	...
	Context("falling back to update when trying to install an existing package install (throwing non-critical error)", func() {
		...
		It(testFailureMsg, func() {
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring(packagedatamodel.ErrPackageAlreadyExists))
		})
		AfterEach(func() {
			options = opts
		})
	...
	Context("failure in updating service account", func() {
		BeforeEach(func() {
			...
			kappCtl.GetClientReturns(crtCtl)
			kappCtl.ListPackagesReturns(testPkgVersionList, nil)
			crtCtl.CreateReturnsOnCall(0, apierrors.NewAlreadyExists(schema.GroupResource{Resource: packagedatamodel.KindServiceAccount}, testServiceAccountName))
	...
	Context("failure in updating cluster admin role", func() {
		BeforeEach(func() {
			...
			crtCtl.CreateReturnsOnCall(1, apierrors.NewAlreadyExists(schema.GroupResource{Resource: packagedatamodel.KindClusterRole}, testClusterRoleName))
	...
	Context("failure in updating cluster role binding", func() {
		BeforeEach(func() {
			...
			crtCtl.CreateReturnsOnCall(2, apierrors.NewAlreadyExists(schema.GroupResource{Resource: packagedatamodel.KindClusterRoleBinding}, testClusterRoleBindingName))
	...
	Context("failure in finding the provided service account", func() {
		BeforeEach(func() {
			...
			kappCtl.GetClientReturns(crtCtl)
			kappCtl.ListPackagesReturns(testPkgVersionList, nil)
			crtCtl.GetReturns(apierrors.NewNotFound(schema.GroupResource{Resource: packagedatamodel.KindServiceAccount}, testServiceAccountName))
		})
		It(testFailureMsg, func() {
			Expect(err).To(HaveOccurred())
	...
	Context("failure in updating secret", func() {
		BeforeEach(func() {
			...
			crtCtl.CreateReturnsOnCall(3, apierrors.NewAlreadyExists(schema.GroupResource{Resource: packagedatamodel.KindSecret}, testSecretName))
	...
	Context("failure when trying to install a package install (with reconciliation failure)", func() {
		BeforeEach(func() {
			...
			kappCtl.GetClientReturns(crtCtl)
			kappCtl.ListPackagesReturns(testPkgVersionList, nil)
			kappCtl.GetPackageInstallReturnsOnCall(0, nil, apierrors.NewNotFound(schema.GroupResource{Resource: packagedatamodel.KindPackageInstall}, testPkgInstallName))
...
func testPackageInstallPostValidation(crtCtl *fakes.CrtClient, kappCtl *fakes.KappClient, expCreatedResourceNames []string) {
...
func testReceive(progress *packagedatamodel.PackageProgress) error {
	for {
		select {
